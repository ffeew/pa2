            ...
            with client_socket:
                while True:
                    match convert_bytes_to_int(read_bytes(client_socket, 8)):
                        ...
                        case 4: # symmetric key establishment
                            size_of_message = convert_bytes_to_int(
                                read_bytes(client_socket, 8)
                            )
                            # session key will be encrypted by RSA
                            encrypted_session_key = read_bytes(client_socket, size_of_message)
                            ...
                            for block_no in range(size_of_message//128):

                                partial_decrypted_file_data = private_key.decrypt(
                                                                # decrypt 128 bytes of data at a time
                                                                encrypted_session_key[block_no*128 : (block_no+1)*128],
                                                                padding.OAEP(
                                ...
                                decrypted_session_key += partial_decrypted_file_data
